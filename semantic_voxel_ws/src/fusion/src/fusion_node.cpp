...
torch::Tensor fusion_points(int col, int row, const torch::Tensor points, const cv::Mat grey_image_label)
{
    ...
    cv::Mat grey_image_label = cv::imread(maskImg);
	*/
...
//    std::cout<<surfaces<<std::endl;
    ...
    **/
    ...
    torch::Tensor image_label = torch::zeros({grey_image_label.rows, grey_image_label.cols}, at::requires_grad(false).dtype(at::kDouble)).contiguous();
    auto p_image_label = image_label.data_ptr<double>();
    for(int i=0; i<grey_image_label.rows; i++)
    {
        for(int j=0; j<grey_image_label.cols; j++)
        {
            unsigned char blue = grey_image_label.at<int>(i,j);
            //GREY
            if(blue == 11) //person
            {
                p_image_label[i*grey_image_label.cols + j] = 2;
            }
            else if(blue == 12) //rider
            {
                p_image_label[i*grey_image_label.cols + j] = 3;
            }
            else if(blue == 18) //bycycle
            {
                p_image_label[i*grey_image_label.cols + j] = 4;
            }
            else if(blue == 13) //car
            {
                p_image_label[i*grey_image_label.cols + j] = 1;
            }
            else
            {
                p_image_label[i*grey_image_label.cols + j] = 0;
            }
        }
    }
	/*
    for(int i=0; i<grey_image_label.rows; i++)
    {
        for(int j=0; j<grey_image_label.cols; j++)
        {
            unsigned char blue = grey_image_label.at<cv::Vec3b>(i,j);
            if(blue <= 19 || blue>=200)
            {
                p_image_label[i*grey_image_label.cols + j] = 0;
            }
	...
	*/
    ...
    for(int num=0; num<num_points; num++)
    {
        ...
        {
//          auto watch = p_image_label[u*grey_image_label.cols + v];
            p_class_score[num] = p_image_label[u*grey_image_label.cols + v];
        }
    }
...
void callback(const sensor_msgs::ImageConstPtr& image_msg, const sensor_msgs::PointCloud2ConstPtr& pCloud)
{
	...
	// image_msg to grey_image_label
	cv::Mat grey_image_label = cv_bridge::toCvShare(image_msg, "mono8")->image;
	torch::Tensor point_Matrix = fusion_points(grey_image_label.cols, grey_image_label.rows, points, grey_image_label);
	fusion::matrix2D_msg pArray_pub;
	/*
	for(int j=0;j<100000;j++)
	{
		if (point_Matrix[j][4].item<float>() >0)
		{
			std::cout<<point_Matrix[j][4].item<float>()<<std::endl;
			break;
		}
	}
	*/
	pArray_pub.header.stamp = pCloud->header.stamp;
	pArray_pub.header.frame_id = "velo_link";
	...
	point_Matrix = point_Matrix.flatten();
	point_Matrix = point_Matrix.contiguous();
	std::vector<float> vec(point_Matrix.data_ptr<float>(),point_Matrix.data_ptr<float>()+point_Matrix.numel());
	pArray_pub.data = vec;
	/*
	for(int j=0;j<100000;j++)
	{
		if (pArray_pub.data[5*j+4]>0)
		{
			std::cout<<pArray_pub.data[5*j+4]<<std::endl;
			break;
		}
	}
	*/
...
int main(int argc, char **argv)
{
  ...
  point_pub = nh.advertise<fusion::matrix2D_msg>("fusion/fusion_points", 100);
  message_filters::Subscriber<sensor_msgs::Image> image_sub(nh,image_topic, 100);
  ...
  sync_.reset(new sync(MySyncPolicy(1000), image_sub, pointcloud_sub));
