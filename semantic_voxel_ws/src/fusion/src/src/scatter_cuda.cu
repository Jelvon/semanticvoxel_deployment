...
//        z_idx[j] = floor( (points_in_voxel[j*5 + 2] +3.f ) / (4.f / fusion_voxel_split));
    ...
    for(int j=0; j<8; j++)
    {
      ...
      if(coordinates[i * 3 + 1] >= 432 )
      {
        ...
        mean = mean / MaskNum;
        bev_map[(channels_per_voxel*j+0)*496*432 + coordinates[i*3 +1]] = mean;
        bev_map[(channels_per_voxel*j+1)*496*432 + coordinates[i*3 +1]] = mean;
        bev_map[(channels_per_voxel*j+2)*496*432 + coordinates[i*3 +1]] = mean;
    }
}
